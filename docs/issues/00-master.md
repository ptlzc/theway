# Roadmap: bring `theway` up to mature agent-framework parity

This is the master tracking issue for closing the gap between the current `theway` (Rust port of
...
1. **Architecture** — where does this slot into the layered stack (`theway-llm-provider` ↔ `theway-core` ↔
   `theway`)? What's the public API surface?
...
- **No sandboxing.** Permission system (#4) is the primary safety layer; the original
  scoped-writes / `.thewayignore` / network-egress sub-issue is closed (was #8).
...
- ~~#8 — Sandboxing: scoped file writes, network egress policy, `.thewayignore`~~ — **de-scoped, closed**
...
- [x] **#22 — Web relay: mount a live session at theway.0xfefe.me/session/&lt;token&gt;**. See
  [[22-web-relay]].
...
- [ ] #13 — `/login`, persistent `~/.theway/auth.json`, OAuth for Codex / Copilot / Google
