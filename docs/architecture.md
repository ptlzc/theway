...
Set `THEWAY_METRICS_ADDR` to a socket address such as `127.0.0.1:9464` to expose Prometheus text format at `GET /metrics`. This listener belongs to the daemon and is independent of the selected client transport. `THEWAY_OBSERVABILITY_QUEUE_CAPACITY` sets the bounded observation queue size and defaults to `4096`; invalid or zero values use the default. `TelemetryConfig::from_env` reads `$THEWAY_DIR/observability.env` as a `KEY=value` fallback below the process environment, so daemons launched by any client pick up OTLP endpoints, headers, and content flags without shell exports.
