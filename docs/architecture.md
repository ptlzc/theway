...
Configuration is documented in [`observability.md`](observability.md): OTLP HTTP/protobuf trace and metric exporters activate from `OTEL_EXPORTER_OTLP_*` variables or the `$THEWAY_DIR/observability.env` runtime file, `THEWAY_METRICS_ADDR` exposes Prometheus, and `THEWAY_OBSERVABILITY_FULL_CONTENT=true` opts into full-context span payloads for Langfuse. Observations are content-safe by default; Prometheus metrics and structured logs stay content-free in both modes.
