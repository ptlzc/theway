# mcp-weather-python — push live weather into theway, let the harness act on it
...
observation into theway as a one-line sentence. Stdlib only — no MCP SDK, no `pip install`.
...
- **The Python server** does only what it is uniquely placed to do — reach the network,
  ...
  (`_meta.theway_summary`). It ships **no policy**: it never decides what "should" happen.
- **theway's harness** decides how that sentence reaches you. This example's `mcp.toml` sets
  ...
  paths are configured in theway, not in the server.
...
1. Copy or merge the `[[server]]` block from `mcp.toml` into one of theway's MCP registries:
   ...
   The `args` path is relative to the cwd theway is launched from. Use an absolute path to the
...
either by exporting before launch (the child inherits theway's environment):

```sh
THEWAY_WEATHER_LOCATION=Tokyo THEWAY_WEATHER_INTERVAL_SECS=300 theway
```
...
```text
when mcp:weather fires notifications/theway/weather/observation, if rain or a storm is
mentioned in the summary, append the summary to /tmp/sf-rain.log; otherwise do nothing
```

theway's `NewTrigger` tool persists the rule to a session sidecar. Each subsequent observation
...
| **One sentence, not a payload** | The reading is collapsed to `_meta.theway_summary`. The raw params (`fetched_at`) are dropped at the adapter (`payload_visibility = Local`); only the sentence survives into the audit and reaches the sub-agent. |
| **Notify once per observation** | `_meta.theway_dedup_key = "obs:<date>:<observation_time>"`. Re-polling the same reading produces the same key, which the runtime dedups — so you get one event per *new* observation, not one per poll. The date prefix keeps the same time-of-day on a later day distinct. The runtime namespaces it as `mcp:weather:custom:obs:<date>:<time>`. |
| **Server-push, no tools** | `tools/list` returns empty; the observation is a JSON-RPC notification (no `id`), routed to theway's notification channel rather than the response router. |
...
3. For non-spec methods, include `_meta.theway_dedup_key` — choose it so re-observing the
   same state produces the same key (dedup) and a genuinely new state produces a new one.
4. Put the human-readable, safe-to-persist string in `_meta.theway_summary`. Leave the
   judgment about what to *do* with it to a theway trigger rule.
