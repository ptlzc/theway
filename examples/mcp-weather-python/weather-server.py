#!/usr/bin/env python3
# MCP server that turns a real-world data source (SF weather) into theway trigger events.
...
#     and collapse the reading into ONE human-readable sentence (`_meta.theway_summary`).
#   * theway's HARNESS does the reasoning: a session trigger rule decides what that sentence
...
#   - notifications/theway/weather/observation  on each new observation, carrying
#     `_meta.theway_dedup_key` (keyed on the observation timestamp, so re-polling the same
#     reading does NOT re-fire) and `_meta.theway_summary` (the one-line weather sentence).
#
# Config via env (all optional):
#   THEWAY_WEATHER_LOCATION       location query for wttr.in   (default "San Francisco")
#   THEWAY_WEATHER_INTERVAL_SECS  poll interval in seconds      (default 600)
#
# Wiring: see `mcp.toml` alongside this file. Run `theway`, then `/triggers` in the REPL.
...
SERVER_NAME = "theway-weather"
SERVER_VERSION = "0.1.0"

LOCATION = os.environ.get("THEWAY_WEATHER_LOCATION", "San Francisco")
POLL_INTERVAL_SECS = int(os.environ.get("THEWAY_WEATHER_INTERVAL_SECS", "600"))
    ...
    while not stop_event.is_set():
        try:
            ...
            if dedup_key != last_key:
                ...
                send_notification(
                    "notifications/theway/weather/observation",
                    {
                        "_meta": {
