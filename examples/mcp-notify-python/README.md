...
| **Server-push notifications** | `notifications/theway/demo/heartbeat` is emitted on a background thread every 10s. JSON-RPC notifications have no `id` field — that's how theway's read pump (`crates/mcp/src/client.rs`) routes them to the `take_notifications()` channel instead of the response router. |
