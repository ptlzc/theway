...
| 缺陷 | 修复 | 文件 |
|------|------|------|
| Health Watch 非持续流 | `HealthService::watch` 从单帧改为 5s 间隔持续流 | `crates/server/src/transport/grpc.rs` |
| 无 SIGTERM handler | `run_transport_loop` select! 增加 SIGTERM 分支 | `crates/server/src/ui/web_loop.rs` |
