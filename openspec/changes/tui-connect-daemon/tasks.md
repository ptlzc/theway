...
- [ ] 1.1 Add `switch_session` RPC to the gRPC service: proto stub extension,
...
- [ ] 1.2 Add `client` module in `theway-transport`: `GrpcClient` wrapper
...
- [ ] 3.4 Local surfaces kept: `/login` (auth.json), `--list-sessions` /
      export/import / delete (local SQLite repo) — verify unchanged behavior
- [ ] 3.5 Startup orchestration in `theway` main: probe port file / default
      port → reuse running daemon, or spawn `thewayd` (inherit cwd/env;
      `-c`/`--resume-id` become launch args) → wait ready → connect

## 4. Cleanup and verification

- [ ] 4.1 Remove now-dead TUI code (web_loop, transport methods, kernel
