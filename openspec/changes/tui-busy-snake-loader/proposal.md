...
用户澄清：**9 个点保留，总高度压到 1 行**（≤ working 高度 120%）；样子
...
- **尺寸**：9 点排成 1 行（9 列 × 1 行轨道），总高度 = 1 行 = working 标签
  高度（≤120% 约束）；`BUSY_STATUS_ROWS` 删除——busy 带从 3 行缩为 **1 行**
...
- busy 时一条彩虹蛇在 9 点轨道左右跑动，蛇尾渐变跟随蛇头、折返；
  busy 带 1 行（与 idle 同高，无跳动），轮子高度 = working 高度 ≤120%。
