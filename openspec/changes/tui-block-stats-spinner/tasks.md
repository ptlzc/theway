...
graph TD
  ...
  E --> G
  G --> F["5-verify: 单测 + tmux e2e + 门禁 + 提交推送"]
```
...
- `5-verify` — `make check` + `make test` + `make lint` + `make fmt-check`；
  ...
  滚动、composer 滚轮、拖拽调高回归、dag 状态带渲染）；选区保持现有高亮不动；
  按 crate 提交推送；close issue #38。
