// Vendored verbatim from mermaid-rs-renderer 0.3.1 (MIT) — keep the diff
// against upstream zero, so upstream clippy noise is allowed here rather
// than "fixed" (that would diverge the vendored copy).
#![allow(
