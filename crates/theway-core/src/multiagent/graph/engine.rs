...
const MAX_TERMINAL_RUNS: usize = 50;
...
impl DagEngine {
    ...
    /// [`MAX_TERMINAL_RUNS`]. Running runs are never removed. Returns the ids
    /// of the evicted runs (each already persisted via [`Self::notify_persist`]).
    pub(super) fn evict(&self, session_id: Option<&str>) -> Vec<String> {
    ...
    /// Remove the terminal runs of `session_id`, keeping the newest `keep`
    /// of them. Running runs are never removed. Returns the number of runs
    /// removed; each is persisted via [`Self::notify_persist`].
    ///
    ...
    /// (Completed/Failed/Cancelled). Returns whether it was removed. The
    /// removal is persisted via [`Self::notify_persist`].
    ...
    pub fn on_goal_tick(
        ...
        let (events, run_session) = {
            ...
            let run_session = run.session_id.clone();
        ...
        if done {
            ...
            self.evict(run_session.as_deref());
    ...
    pub fn cancel_run(&self, run_id: &str, reason: Option<&str>) {
        let (running, cancelled, session_id) = {
            ...
            run.completed_at = Some(now_ms());
            emit_state(run);
            (running_ids, cancelled_ids, run.session_id.clone())
        };
