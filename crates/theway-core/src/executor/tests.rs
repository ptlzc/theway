...
impl FakeExecutor {
    fn new(kind: ExecutorKind) -> Self {
        Self { kind, fail: FailMode::None, calls: Mutex::new(Vec::new()) }
    }

    fn failing(kind: ExecutorKind, fail: FailMode) -> Self {
        Self { kind, fail, calls: Mutex::new(Vec::new()) }
    }
...
impl ToolExecutor for FakeExecutor {
    ...
    async fn run_command(
        ...
        Ok(CommandOutput { stdout: "out".into(), stderr: "err".into(), exit_code: 0 })
    }
    ...
    async fn git(&self, args: &[String]) -> Result<CommandOutput> {
        ...
        Ok(CommandOutput { stdout: "git-out".into(), stderr: String::new(), exit_code: 0 })
    }
}
...
async fn read_file_records_call_and_returns_content() {
    let executor = FakeExecutor::new(ExecutorKind::Local);
    let content = executor.read_file(Path::new("/w/src/lib.rs")).await.unwrap();
...
async fn run_command_records_cwd_argv_and_timeout() {
    ...
    assert_eq!(
        output,
        CommandOutput { stdout: "out".into(), stderr: "err".into(), exit_code: 0 }
    );
...
async fn list_dir_grep_find_record_queries_and_return_entries() {
    ...
    assert_eq!(executor.grep("fn main", Path::new("/w")).await.unwrap(), vec!["match-line"]);
    assert_eq!(executor.find("**/*.rs", Path::new("/w")).await.unwrap(), vec!["src/main.rs"]);
    assert_eq!(
        executor.calls(),
        vec![
            r#"list_dir:"/w""#,
...
async fn git_records_args_and_returns_output() {
    let executor = FakeExecutor::new(ExecutorKind::Local);
    let output = executor.git(&args(&["status", "--porcelain"])).await.unwrap();
...
async fn unsupported_kind_error_propagates_like_sandbox_stub() {
    ...
    assert!(matches!(error, ExecutorError::UnsupportedKind(ExecutorKind::Sandbox)));
...
async fn executor_side_error_propagates_with_message() {
    let executor = FakeExecutor::failing(
        ExecutorKind::Local,
        FailMode::Other("spawn failed".into()),
    );
    let error = executor
...
async fn dyn_trait_object_dispatches_and_is_shareable_across_threads() {
    ...
    let handle = std::thread::spawn(move || {
        futures::executor::block_on(async move {
            ...
            assert_eq!(cloned.read_file(Path::new("/w/x")).await.unwrap(), "fake-content");
        });
    });
    handle.join().unwrap();
}
...
fn command_output_success_reflects_exit_code() {
    let ok = CommandOutput { stdout: String::new(), stderr: String::new(), exit_code: 0 };
    let failed = CommandOutput { stdout: String::new(), stderr: "boom".into(), exit_code: 1 };
    assert!(ok.success());
