...
This file contains the complete crate-local modification rules for `theway-core`. Read the [crate overview](README.md) and [runtime architecture](docs/architecture.md) before changing agent, session, or multiagent behavior.
...
- Keep typed session interpretation in `agent/session`; persistence implementations receive only `theway-contract` records through `PersistentSessionStorage`.
...
- Keep multi-file suites under `tests/<mirrored-src-path>/` and bridge them from the owning source module; do not place them under `src/`.
