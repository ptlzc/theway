...
impl ThewayGrpc for GrpcState {
    ...
    async fn graph_checkpoint(
        ...
        let runs: Vec<theway_core::harness::graph_engineering::types::DagRun> = match request.run_id
        {
            Some(run_id) => self
                .dag_engine
                .get_run(&run_id)
...
mod tests {
    ...
    async fn get_node_output_returns_fragment_from_offset() {
        ...
        let job_id = state
            .registry
            .register(theway_core::harness::subagents::registry::JobInit {
                agent: "explorer".into(),
                source: "dag".into(),
    ...
    async fn stream_events_forwards_dag_node_status_frames() {
        ...
        state
            ...
            .send(DagEvent::NodeStatus {
                run_id: "goal-1".into(),
                node_id: "main".into(),
                status: theway_core::harness::graph_engineering::types::NodeStatus::Failed,
