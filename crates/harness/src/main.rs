//! theway — coding agent CLI binary. Thin assembly layer on top of the
//! `theway` SDK crate (same package's `lib.rs`); all runtime logic lives
//! in the library so external projects can embed it in-process.
//!
...
use theway_core::harness::graph_engineering::persist::{
...
async fn run_repl(mut cli: Cli, cwd: std::path::PathBuf, repo: JsonlSessionRepo) -> Result<()> {
    ...
    let subagent_registry = theway_core::harness::subagents::registry::SubagentJobRegistry::new();
    dag_engine.set_launcher(Some(tools::node_launcher::node_launcher(
        dag_engine.clone(),
        model.clone(),
