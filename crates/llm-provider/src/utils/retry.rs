//! Provider-level HTTP retry. Mirrors the per-provider SDK retry behaviour that the TS theway-llm-provider
    ...
    loop {
        ...
        match result {
            ...
            Ok(resp) => {
                ...
                tracing::debug!(
                    target: "theway_llm_provider::retry",
...
fn is_retryable_status(status: reqwest::StatusCode) -> bool {
    ...
    // history; theway always sends the full history, so a plain retry is that replay.
