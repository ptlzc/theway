...
//! [`theway::app::daemon::DaemonApp`] through the `TransportHost` surface.
//!
//! ```
...
async fn main() -> Result<()> {
    ...
    let local_models = theway::local_models::load_all(&cwd, cli.base_url.as_deref()).await?;
    ...
    let mut model = match theway::model::auto_detect_model(
        ...
        Err(e) if cli.provider.is_none() && cli.model.is_none() => {
            ...
            theway::model::credential_less_default()
        }
        Err(e) => return Err(e),
    };
    ...
    dag_engine.set_launcher(Some(theway::tools::node_launcher(
        dag_engine.clone(),
        model.clone(),
    ...
    let restored_dags =
        dag_engine.restore(theway::dag_persist::load_session_runs(&cwd, &session_id).await);
    ...
    let _dag_persist =
        theway::dag_persist::DagPersistHandle::spawn(dag_engine.clone(), cwd.clone());
    let mut tools = theway::tools::session_tool_set(
        &memory_dir,
        &dag_engine,
        &subagent_registry,
        &model,
    ...
    let mcp = theway::mcp_loader::load_all(&cwd).await;
    ...
    let ts_extensions = theway::ts_extensions::ExtensionRegistry::discover(&cwd);
    for error in &ts_extensions.errors {
        tracing::warn!(target: "extensions", "{error}");
    }
    let compact_algorithms = Arc::new(theway::ts_extensions::compact_algorithm_registry(
        &ts_extensions,
    ));
    ...
    let resolved_builtins =
        theway::builtin_skills::resolve_builtins(&cli.builtin_skill, &config_enabled_builtins)?;
    let mut combined_skills = theway::builtin_skills::merge_with_user_project(
        resolved_builtins.skills.clone(),
        &loaded_skills.skills,
    );
    {
        let state = theway::skill_overrides::load(&config::base_dir()).await;
        theway::skill_overrides::apply(&state, &mut combined_skills);
    }
    ...
    let reload_skills_fn: theway_core::ReloadSkillsFn = {
        ...
        Arc::new(move || {
            ...
            Box::pin(async move {
                ...
                let mut merged =
                    theway::builtin_skills::merge_with_user_project(builtins, &loaded.skills);
                let state = theway::skill_overrides::load(&config::base_dir()).await;
                theway::skill_overrides::apply(&state, &mut merged);
    ...
    let (control_plane_hook, control_plane_prompt_rx) = if cli.always_allow || cli.yes {
        (Some(theway::control_plane_prompt::allow_hook()), None)
    } else {
        let (hook, rx) = theway::control_plane_prompt::interactive_hook();
        (Some(hook), Some(rx))
    };
    ...
    let lsp_supervisor = Arc::new(theway::lsp_supervisor::LspSupervisor::load(&cwd).await);
    let lsp_lang_count = lsp_supervisor.language_count();
    let after_tool_call = if lsp_supervisor.is_empty() {
        None
    } else {
        Some(theway::lsp_supervisor::as_after_tool_call(
    ...
    let trigger_executor = Arc::new(theway::trigger_engine::execution::TriggerExecutor::new(
        harness.agent_arc(),
    ...
    let _unsub_trigger = trigger_executor.subscribe(theway::app::listener::trigger_listener(
        feed_tx.clone(),
        cli.debug,
    ));
