...
fn sample_trigger(idempotency_key: &str, trace_id: &str) -> theway_core::Trigger {
    theway_core::Trigger {
        source: theway_core::TriggerSource::Mcp {
        ...
        payload_visibility: theway_core::PayloadVisibility::Local,
        payload_summary: Some("PR #42 merged".into()),
        payload: None,
        idempotency_key: idempotency_key.into(),
        replacement_policy: theway_core::ReplacementPolicy::Drop,
        trace_id: trace_id.into(),
        authority: theway_core::TriggerAuthority {
            principal_id: "mcp:github".into(),
            principal_label: "github".into(),
            credential_scope: theway_core::CredentialScope::Project,
...
async fn handle_trigger_accept_persists_audit_custom_entry_with_accepted_state() {
    ...
    let outcome = harness
        ...
        .await;
    assert!(matches!(outcome, theway_core::EvaluationOutcome::Accept));
    ...
    let trigger_entries: Vec<_> = entries
        ...
        .filter_map(|e| match e {
            SessionTreeEntry::Custom {
                custom_type, data, ..
            } if custom_type == theway_core::TriggerRecord::CUSTOM_TYPE => Some(data.clone()),
            _ => None,
        })
        .collect();
    assert_eq!(
    ...
    let record: theway_core::TriggerRecord =
        serde_json::from_value(data.clone()).expect("audit payload must decode as TriggerRecord");
    assert_eq!(record.state, theway_core::TriggerState::Accepted);
    ...
    assert_eq!(state, theway_core::TriggerState::Accepted);
    assert!(
        audit_id.is_some(),
...
async fn handle_trigger_dedup_emits_deduped_state_and_persists_record() {
    ...
    let prev_trace_id = match second {
        theway_core::EvaluationOutcome::Deduped {
            previous_trace_id, ..
        } => previous_trace_id,
    ...
    let states: Vec<_> = entries
        ...
        .filter_map(|e| match e {
            SessionTreeEntry::Custom {
                custom_type, data, ..
            } if custom_type == theway_core::TriggerRecord::CUSTOM_TYPE => {
                let r: theway_core::TriggerRecord =
...
async fn handle_trigger_cycle_suppression_persists_cycle_suppressed_state() {
    ...
    let session = Session::new(storage.clone() as Arc<dyn SessionStorage>);
    let mut opts = AgentHarnessOptions::new(faux_model(), session.clone());
    opts.trigger_runtime = theway_core::TriggerRuntimeConfig {
    ...
    assert!(matches!(
        suppressed,
        theway_core::EvaluationOutcome::CycleSuppressed { .. }
    ));
    ...
    assert_eq!(last_state, theway_core::TriggerState::CycleSuppressed);
}
...
async fn handle_trigger_persistence_failure_still_returns_outcome_and_emits_error() {
    ...
    let outcome = harness
        ...
        .await;
    assert!(
        matches!(outcome, theway_core::EvaluationOutcome::Accept),
...
async fn register_notification_hook_drives_pump_into_handle_trigger() {
    ...
    struct CountedHook {
        label: String,
        triggers: std::sync::Mutex<Vec<theway_core::Trigger>>,
    }
    #[async_trait::async_trait]
...
async fn before_trigger_deny_records_permission_denied_state_and_reason() {
    use theway_core::{BeforeTriggerContext, BeforeTriggerDecision, BeforeTriggerHook};
    ...
    assert_eq!(record.state, theway_core::TriggerState::PermissionDenied);
    let decision = record
        .evaluator_decision
        .as_ref()
    ...
    let event_decision = evs
        ...
        .find_map(|e| match e {
            HarnessEvent::TriggerHandled {
                ...
                evaluator_decision,
                ..
            } if *state == theway_core::TriggerState::PermissionDenied => {
...
async fn before_trigger_prompt_records_needs_approval_state_and_reason() {
    ...
    assert_eq!(record.state, theway_core::TriggerState::NeedsApproval);
    assert_eq!(
...
async fn before_trigger_prompt_allow_admits_trigger_and_binds_source_identity() {
    ...
    trigger.source_kind = theway_core::SourceKind::Mcp;
    trigger.source_label = "external notifier".into();
    trigger.event_label = "notification".into();
    trigger.payload_visibility = theway_core::PayloadVisibility::Shared;
    ...
    assert_eq!(trigger_record.state, theway_core::TriggerState::Accepted);
...
async fn event_ordering_handled_then_started_then_completed() {
    ...
    for (i, e) in evs.iter().enumerate() {
        match e {
            HarnessEvent::TriggerHandled {
                trace_id,
                state: theway_core::TriggerState::Accepted,
                ..
...
) -> theway_core::BeforeTriggerActionHook {
    use theway_core::{
        ...
        Arc::new(move |ctx: BeforeTriggerActionContext, _cancel| {
            ...
            Box::pin(async move {
                TriggerAction {
                    ...
                    delivery: theway_core::TriggerDelivery::SubAgent,
                }
            })
        });
    hook
...
fn inject_summary_action_hook() -> theway_core::BeforeTriggerActionHook {
    use theway_core::{
        BeforeTriggerActionContext, BeforeTriggerActionHook, PromoteAction, TriggerAction,
        TriggerDelivery,
    };
...
fn inject_and_run_action_hook(prompt: &'static str) -> theway_core::BeforeTriggerActionHook {
    use theway_core::{
        BeforeTriggerActionContext, BeforeTriggerActionHook, PromoteAction, TriggerAction,
        TriggerDelivery,
    };
...
fn promotion_condition_any_of_returns_intersection_on_match() {
    use theway_core::PromotionCondition;
...
fn promotion_condition_any_of_fails_closed_when_pointer_missing() {
    use theway_core::{PromotionCondition, PromotionConditionSkipReason};
...
async fn promote_when_result_details_match_does_not_consult_summary() {
    use theway_core::{BeforeTriggerActionContext, PromoteAction, PromotionCondition};
    ...
    opts.before_trigger_action = Some({
        let hook: theway_core::BeforeTriggerActionHook =
            Arc::new(move |ctx: BeforeTriggerActionContext, _cancel| {
                Box::pin(async move {
                    theway_core::TriggerAction {
                        prompt: format!(
                            "{} fired: {}",
                            ctx.trigger.source_label, ctx.trigger.event_label
                        ),
...
async fn promote_while_parent_is_streaming_routes_through_follow_up_single_write() {
    ...
    opts.before_trigger_action = Some({
        let hook: theway_core::BeforeTriggerActionHook = Arc::new(
            move |ctx: theway_core::BeforeTriggerActionContext, _cancel| {
                Box::pin(async move {
                    theway_core::TriggerAction {
                        prompt: format!(
                            "{} fired: {}",
                        ...
                        promote: theway_core::PromoteAction::PromoteSummaryNow {
                            template_body: None,
                        },
                        promote_requires_approval: false,
                        delivery: theway_core::TriggerDelivery::SubAgent,
