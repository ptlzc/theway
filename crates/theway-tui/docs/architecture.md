...
The status area remains one row while a turn is busy. [`ui/snake_loader.rs`](../src/ui/snake_loader.rs) renders the Pi-compatible Braille sequence `⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏` in one fixed terminal cell. Each frame changes both the Braille mask and its true-color rainbow hue, while the adjacent `working` label keeps one fixed color and no font modifier. The elapsed time and live character-rate/input/output statistics form one left-aligned cluster after `working`; queue and scroll indicators follow that cluster instead of anchoring throughput at the terminal's right edge. The live character-rate meter selects one of five animation intervals from 130 ms down to 10 ms; the glyph position and surrounding status layout stay fixed when that interval changes.

[`ui/app_input/mod.rs`](../src/ui/app_input/mod.rs) and [`ui/app_input/history.rs`](../src/ui/app_input/history.rs) own composer input, completion, history, paste, and submission. The editor state comes from `theway-ratatui-textarea`; terminal rendering helpers and link/scrollbar behavior come from `theway-pager-render`.
