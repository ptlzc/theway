...
Turn 忙碌期间，状态区域仍保持一行。[`ui/snake_loader.rs`](../src/ui/snake_loader.rs) 在一个固定 terminal cell 中渲染与 Pi 兼容的盲文序列 `⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏`。每一帧同时改变盲文点阵和 true-color 彩虹色相。实时字符速率计从 130 ms 到 10 ms 的五档动画间隔中选择速度；档位变化时字符位置与周围状态布局保持不变。
