...
本 crate 依赖 `theway-transport`、`theway-storage` 和渲染 widget，但绝不依赖 `theway-core` 或 `theway-daemon`。运行时 turn、trigger、暴露给模型的工具和编排状态由 daemon 负责。
...
- `theway-ratatui-textarea` 提供 composer 编辑器。
- `theway-pager-render` 提供宽度、scrollbar、颜色、路径和 OSC 8 链接辅助逻辑。
