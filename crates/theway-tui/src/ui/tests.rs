use super::{App, AppConfig};
    ...
    use theway_transport::grpc::{serve_grpc, GrpcState};
    use theway_transport::testing::{FakeSessionOps, empty_sidebar_snapshot};
    ...
    async fn test_app() -> (App, mpsc::UnboundedReceiver<WireCommand>) {
        ...
        let app = App::new(AppConfig {
            ...
            session_repo: Arc::new(theway::SqliteSessionRepo::new(
    ...
    async fn snapshot_rebuilds_feed_and_resyncs_busy_panel() {
        let (mut app, _rx) = test_app().await;
        assert!(app.feed.lines(100).iter().any(|l| {
            l.spans
                .iter()
                .any(|s| s.content.contains("banner"))
        }));
    ...
    async fn slash_quit_sets_quit_and_clear_empties_feed() {
        let (mut app, _rx) = test_app().await;
        app.dispatch_slash("/quit", &mut terminal_placeholder()).await;
        assert!(app.quit);

        app.feed.push_user("stale");
        app.dispatch_slash("/clear", &mut terminal_placeholder()).await;
    ...
    async fn model_picker_alt_m_selects_and_sends_set_model() {
        ...
        assert!(app
            .handle_model_picker_key(&KeyEvent::new(KeyCode::Enter, KeyModifiers::empty()))
            .await);
        assert!(app
            .handle_model_picker_key(&KeyEvent::new(KeyCode::Enter, KeyModifiers::empty()))
            .await);
