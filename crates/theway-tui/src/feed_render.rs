...
fn underline_links(lines: &mut [Line<'static>]) {
    ...
    for link in overlay.links() {
        if let Some(line) = lines.get_mut(link.screen_row as usize) {
    ...
    for span in std::mem::take(&mut line.spans) {
        ...
        if span_end <= start_col || col >= end_col {
            ...
            let pre = span
                ...
                .map(|s| ratatui::text::Span::styled(s.to_string(), span.style));
            let mid = span
                .content
                .get(
