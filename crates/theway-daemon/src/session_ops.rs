...
impl SessionOps for AppSessionOps {
    async fn list(&self) -> Result<Vec<SessionSummary>> {
        ...
        for path in self.repo.list().await.map_err(repo_err)? {
            ...
            let name = session
                ...
                .flatten()
                .unwrap_or_default();
            let preview = theway::session::first_user_text(&session).await;
    ...
    async fn rename(&self, id: &str, name: &str) -> Result<()> {
        ...
        let path = theway::session::find_path_by_id(&self.repo, id)
            .await?
    ...
    async fn delete(&self, id: &str) -> Result<Vec<String>> {
        ...
        theway::session::delete_by_id(&self.repo, id).await?;
        Ok(Vec::new())
    }
}
