...
impl TelemetryHandle {
    ...
    pub async fn from_config(config: TelemetryConfig) -> Self {
        ...
        let (tracer_provider, meter_provider, tracer, otel_metrics) =
            if config.otlp_enabled || config.otlp_metrics_enabled {
                match build_otel(&config, status.clone()) {
                    Ok(parts) => parts,
                    Err(error) => {
                        status.record_failure(format!("OpenTelemetry export is disabled: {error}"));
                        tracing::warn!(
                            target: "theway::observability",
                            %error,
        ...
        let metrics_server = match config.metrics_addr {
            Some(addr) => {
                MetricsServer::spawn(addr, observer.metrics.prometheus.registry(), status.clone())
                    .await
            }
            None => None,
        };
    ...
    /// Observer health shared with the transport status plane; the TUI renders
    /// it as a hint when degraded.
    pub fn status(&self) -> Arc<ObservabilityStatus> {
        self.status.clone()
    }
...
impl DaemonRuntimeObserver {
    fn record_drop(&self) {
        ...
        self.status.record_failure(format!(
            "observability queue full: {dropped} observation(s) dropped"
        ));
    }
}
...
#[derive(Debug)]
struct StatusReportingSpanExporter {
...
#[derive(Debug)]
struct StatusReportingMetricExporter {
...
fn report_export_result(status: &Arc<ObservabilityStatus>, prefix: &str, result: &OTelSdkResult) {
    match result {
...
fn build_otel(
    ...
    let (tracer_provider, tracer) = if config.otlp_enabled {
        ...
        let span_exporter = StatusReportingSpanExporter {
            inner: span_exporter,
            status: status.clone(),
        };
    ...
    let (meter_provider, metrics) = if config.otlp_metrics_enabled {
        ...
        let metric_exporter = StatusReportingMetricExporter {
