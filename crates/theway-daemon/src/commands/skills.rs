...
async fn install_skill(argv: &[String], ctx: &CommandCtx<'_, DaemonCtx>) -> CommandOutcome {
    ...
    let tool = theway_daemon::tools::install_skill::InstallSkillTool::new(cell);
    match tool
        .execute(
...
async fn set_skill_enabled(
    ...
    if let Err(e) =
        crate::skill_overrides::set_and_save(&theway_transport::client::base_dir(), &name, source, enabled)
            .await
    {
...
async fn remove_skill(argv: &[String], ctx: &CommandCtx<'_, DaemonCtx>) -> CommandOutcome {
    ...
    let cell = skill_harness_cell(ctx);
    let tool = theway_daemon::tools::remove_skill::RemoveSkillTool::new(cell);
    match tool
        .execute(
            "slash-skills-remove",
            params,
...
fn skill_harness_cell(
    ctx: &CommandCtx<'_, DaemonCtx>,
) -> theway_daemon::tools::skill::SkillHarnessCell {
