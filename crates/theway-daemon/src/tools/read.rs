...
impl AgentTool for ReadTool {
    ...
    async fn execute(
        ...
        let full_slice: String = taken_lines.concat();
        let (slice, trunc) = truncate_head(&full_slice, limit, DEFAULT_MAX_BYTES);
...
// The suite exercises the local executor's host side effects; the executor is
// always compiled (issue #123), and sandbox mode omits these tools at runtime.
#[cfg(test)]
mod tests {
    use super::*;
