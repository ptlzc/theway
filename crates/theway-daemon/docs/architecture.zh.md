...
`theway-daemon` 是工作区唯一直接消费 [`theway-core`](../../theway-core/docs/architecture.md) 的 crate。它提供具体宿主行为，并把 core 状态适配到持久化和协议 crate。可复用运行时机制留在 core；wire 表示留在 [`theway-transport`](../../theway-transport/docs/architecture.md)；终端交互留在 [`theway-tui`](../../theway-tui/README.md)。
...
本地适配器使用 [`theway-storage`](../../theway-storage/docs/architecture.md)。`RemoteRuntimeStorage` 使用 [`theway-transport`](../../theway-transport/docs/architecture.md) 的存储 RPC 操作。编排代码依赖这些 daemon trait，不暴露 SQLite 类型。
...
[`hooks/mod.rs`](../src/hooks/mod.rs)、[`hook_executors.rs`](../src/hook_executors.rs)、[`trigger_engine/mod.rs`](../src/trigger_engine/mod.rs) 和 [`triggers/mod.rs`](../src/triggers/mod.rs) 负责进程/webhook 操作、动态 trigger 轮询与提升、cron 执行和 notification 投递。持久化 sidecar 记录来自 [`theway-contract`](../../theway-contract/docs/architecture.md)，调度与投递策略留在本 crate。

[`mcp_loader.rs`](../src/mcp_loader.rs) 使用 [`theway-mcp`](../../theway-mcp/docs/architecture.md) 发现外部 MCP 工具与 notification。[`mcp_server.rs`](../src/mcp_server.rs) 将 daemon 暴露为 MCP server。[`lsp_supervisor.rs`](../src/lsp_supervisor.rs) 负责 language server 进程生命周期。
