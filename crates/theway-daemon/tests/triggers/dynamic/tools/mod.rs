...
fn definitions_and_labels_are_catalog_names() {
    let new = NewTriggerTool;
    let list = ListTriggersTool;
    let remove = RemoveTriggerTool;
    let state = SetTriggerStateTool;
...
fn new_trigger_permission_reason_is_shape_only() {
    let cls = NewTriggerTool.permission_classification(
    ...
    let spec_cls = NewTriggerTool.permission_classification(&json!({ "spec": "spec text" }));
    match spec_cls {
        PermissionClassification::Prompt { reason } => {
    ...
    let fallback = NewTriggerTool.permission_classification(&json!({}));
    match fallback {
        PermissionClassification::Prompt { reason } => {
...
fn remove_trigger_permission_reason_distinguishes_single_and_all() {
    let single = RemoveTriggerTool.permission_classification(&json!({ "id": "dyn-1" }));
    match single {
        PermissionClassification::Prompt { reason } => assert!(reason.contains("`dyn-1`"), "{reason}"),
        other => panic!("must prompt, got {other:?}"),
    }

    let all = RemoveTriggerTool.permission_classification(&json!({ "all": true }));
    match all {
        PermissionClassification::Prompt { reason } => assert!(reason.contains("ALL"), "{reason}"),
        other => panic!("must prompt, got {other:?}"),
    }

    let fallback = RemoveTriggerTool.permission_classification(&json!({}));
    match fallback {
...
fn set_trigger_state_permission_allows_disable_and_prompts_enable() {
    let disable = SetTriggerStateTool.permission_classification(&json!({ "enabled": false }));
    assert!(matches!(disable, PermissionClassification::Allow));

    let enable = SetTriggerStateTool.permission_classification(
