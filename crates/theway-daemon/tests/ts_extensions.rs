//! Integration tests for the CLI TS extension host (`theway_daemon::ts_extensions`, moved out of
...
async fn compaction_adapter_dispatches_through_ts_hooks() {
    ...
    let request = theway_core::agent::compaction::algorithm::SummarizeRequest {
        model: &model,
        messages: &messages,
