...
async fn handle_trigger_accept_persists_audit_custom_entry_with_accepted_state() {
    ...
    let record: theway_daemon::trigger_engine::types::TriggerRecord =
...
async fn handle_trigger_dedup_emits_deduped_state_and_persists_record() {
    ...
    let prev_trace_id = match second {
        theway_daemon::trigger_engine::runtime::EvaluationOutcome::Deduped {
            previous_trace_id,
            ..
        } => previous_trace_id,
    ...
    assert_eq!(
        ...
        vec![
            theway_daemon::trigger_engine::types::TriggerState::Accepted,
            theway_daemon::trigger_engine::types::TriggerState::Deduped
        ],
...
async fn handle_trigger_cycle_suppression_persists_cycle_suppressed_state() {
    ...
    assert!(matches!(
        suppressed,
        theway_daemon::trigger_engine::runtime::EvaluationOutcome::CycleSuppressed { .. }
    ));
    ...
    assert_eq!(
        last_state,
        theway_daemon::trigger_engine::types::TriggerState::CycleSuppressed
    );
}
...
async fn register_notification_hook_drives_pump_into_handle_trigger() {
    use theway_daemon::trigger_engine::notification_hook::{
    ...
    struct CountedHook {
        label: String,
        triggers: std::sync::Mutex<Vec<theway_daemon::trigger_engine::types::Trigger>>,
    }
        ...
        .filter(|e| {
            matches!(
                ...
                SessionTreeEntry::Custom { custom_type, .. }
                    if custom_type == theway_daemon::trigger_engine::types::TriggerRecord::CUSTOM_TYPE
            )
        })
        .count();
