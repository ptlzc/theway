...
fn promotion_condition_any_of_returns_intersection_on_match() {
    use theway_daemon::trigger_engine::execution::PromotionCondition;
...
fn promotion_condition_any_of_fails_closed_when_pointer_missing() {
    use theway_daemon::trigger_engine::execution::{
        PromotionCondition, PromotionConditionSkipReason,
    };
...
async fn promote_when_result_details_match_does_not_consult_summary() {
    ...
    before_trigger_action = Some({
        let hook: theway_daemon::trigger_engine::execution::BeforeTriggerActionHook =
            Arc::new(move |ctx: BeforeTriggerActionContext, _cancel| {
                Box::pin(async move {
                    theway_daemon::trigger_engine::execution::TriggerAction {
                        prompt: format!(
                            "{} fired: {}",
                        ...
                        delivery:
                            theway_daemon::trigger_engine::execution::TriggerDelivery::SubAgent,
                    }
                })
            });
        hook
