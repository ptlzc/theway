...
fn scrollbar_styles(is_following: bool) -> (Style, Style) {
    // theway localization: fixed grayscale styles (the donor reads a global
    // theme; this crate carries no theme system).
    if is_following {
        // Very dim - scrollbar is subtle when following
        let track_style = Style::new().bg(Color::Rgb(30, 30, 35));
        ...
        let track_style = Style::new().bg(Color::Rgb(45, 45, 52));
        let thumb_style = Style::new()
...
mod tests {
    ...
    fn test_render_scrollbar_following_vs_not() {
        ...
        // theway localization: fixed styles are always distinguishable
        // (no color-support detection in this crate).
