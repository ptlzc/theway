...
- Added `/goal <condition>` session goal mode. After each successful turn, theway runs a
  no-tools evaluator over a bounded transcript; if the transcript lacks explicit evidence
  for the goal, theway injects a continuation prompt with the missing evidence reason. `/goal`,
...
- **#32** Optional bundled `karpathy-guidelines` built-in skill. Off by default; enable
  ...
  `~/.theway/config.toml` `[builtin_skills] enabled = ["karpathy-guidelines"]`. CLI and config
...
- **#37** Local OpenAI-compatible model configs. `theway` now loads
  `~/.theway/models.json` and `<cwd>/.theway/models.json`, registers those entries through
  `theway_ai_llm_provider::register_custom_model`, and lets users select them with
...
- **#47** CLI help now advertises accepted values for finite-set options. `--thinking`
  ...
  `theway --help` and invalid-value errors both show the supported set.
...
- `theway --resume <id>` now resumes a specific session directly (same as `--resume-id`);
  bare `--resume` keeps the picker.
- theway starts without a model API key (warning instead of a hard error), so
...
- **#6** `theway --continue` / `-c`, `theway --list-all-sessions`, `/save` (Markdown transcript
...
- **#9** `theway-mcp` crate: stdio transport, JSON-RPC 2.0 framing, initialize handshake,
  `tools/list` + `tools/call`. `McpAgentTool` adapter wraps server tools as `AgentTool`s.
  `~/.theway/mcp.toml` loader spawns each server lazily.
- **#10 Part A** Dual-root skills loader (`<cwd>/.theway/skills/` overrides `~/.theway/skills/`),
...
- **#15** Tracing subscriber writing per-session logs to `~/.theway/logs/<session>.log` via
...
- **`InstallSkill` builtin tool (issue #87 sub-PR B)** New harness tool that installs a skill
  into the user-global skills directory (`~/.theway/skills/<name>/SKILL.md`) from one of three
  ...
  refreshes without a `theway` restart. Two-phase safety model: the first tool call returns a
...
- **Skill catalog hot-reload (issue #87 sub-PR A)** New `AgentHarnessOptions::reload_skills_fn:
  ...
  `InstallSkillTool` / `/skills reload`) refresh the skill catalog without restarting `theway`,
...
- **#17** `HarnessEvent` typed bus on `AgentHarness` (SessionStart / Compaction /
  Branch). Prompt-template file loader (`<cwd>/.theway/templates/` overrides
  `~/.theway/templates/`) + `/template <name> [k=v ...]` slash command.
...
27 test binaries, ~225 tests, `clippy -D warnings` clean across `theway-llm-provider`,
`theway-core`, `theway`, `theway-mcp`.
